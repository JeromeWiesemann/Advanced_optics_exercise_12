...
tau = 20e-10 # 1/s
sol = odeint(model, y_0, t)
plt.plot(t, v(sol[:, 0]), label = r"$\tau = 20\cdot 10^{-10}$")
# plt.plot(t, sol[:, 1], label = r"$\tau = 20\cdot 10^{-10}$")

tau = 20e-14 # 1/s
sol = odeint(model, y_0, t)
plt.plot(t, v(sol[:, 0]), label = r"$\tau = 20\cdot 10^{-14}$")
# plt.plot(t, sol[:, 1], label = r"$\tau = 20\cdot 10^{-14}$")

tau = 20e-15 # 1/s
# plt.plot(t, sol[:, 0], label = "N1")
sol = odeint(model, y_0, t)
plt.plot(t, v(sol[:, 0]), label = r"$\tau = 20\cdot 10^{-15}$")
# plt.plot(t, sol[:, 1], label = r"$\tau = 20\cdot 10^{-15}$")


plt.ylabel(r"Velocity $v\,[\si{\m\per\s}]$")
plt.xlabel(r"Time $[\si{\s}]$")

plt.legend()
plt.savefig("Velocity_over_time_multiple_tau.pdf", dpi = 300)
plt.show()